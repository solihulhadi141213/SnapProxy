<?php
    ...
    $deleteExpiredCaptchas = $Conn->prepare("DELETE FROM token_session WHERE token_category = ? AND timestamp_expired < ?");
    $captchaCategory = 'Captcha';
    $deleteExpiredCaptchas->bind_param("ss", $captchaCategory, $timestamp_now);
    $deleteExpiredCaptchas->execute();
    
    // Tipe/Kategori
    $token_category = "Captcha";

    // Parameter guide (kosong)
    $parameter_guide = "";

    // Simpan Captcha Di Database
    $query = "INSERT INTO token_session (
        token_category, 
        parameter_guide, 
        unique_code, 
        timestamp_creat, 
